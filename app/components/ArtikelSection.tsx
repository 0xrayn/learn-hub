...
export default function ArtikelSection() {
  ...
  useEffect(() => {
    fetchArticles().then((data) => {
      setArticles(data);
      setLoading(false);
      // Trigger fade-in setelah data loaded
  ...
  return (
    <section id="artikel" style={{ padding: "96px 16px", opacity: 1 }}>
      <style>{`
        @keyframes fadeUpCard {
          from { opacity: 0; transform: translateY(24px); }
          to   { opacity: 1; transform: none; }
        }
        .artikel-card-wrap { animation: fadeUpCard .55s cubic-bezier(.16,1,.3,1) both; }
        .artikel-card-wrap:nth-child(1) { animation-delay: .05s }
        .artikel-card-wrap:nth-child(2) { animation-delay: .13s }
        .artikel-card-wrap:nth-child(3) { animation-delay: .21s }
        .artikel-card-wrap:nth-child(4) { animation-delay: .29s }
        .artikel-card-wrap:nth-child(5) { animation-delay: .35s }
        .artikel-card-wrap:nth-child(6) { animation-delay: .41s }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: "60px 0", opacity: 0.4 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", border: "2px solid rgba(6,182,212,0.2)", borderTop: "2px solid #06b6d4", margin: "0 auto 12px", animation: "spin 0.7s linear infinite" }} />
            <p style={{ color: "var(--text-main,#e8eaf0)", fontSize: 13 }}>Memuat artikel...</p>
          </div>
        )}
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 0", opacity: 0.4 }}>
            ...
            <p style={{ color: "var(--text-main,#e8eaf0)", fontSize: 14 }}>
              {cat === "Semua" ? "Belum ada artikel. Admin dapat menambahkan lewat dashboard." : `Belum ada artikel kategori "${cat}".`}
            </p>
          </div>
        )}

        {/* Grid — animasi via CSS keyframes, bukan IntersectionObserver */}
        {!loading && filtered.length > 0 && visible && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
            {filtered.map((a) => (
              <div key={a.id} className="artikel-card-wrap">
                <Link href={`/artikel/${a.id}`} style={{ textDecoration: "none", display: "block", height: "100%" }}>
                  <article
                    className="grad-border overflow-hidden"
                    style={{ cursor: "pointer", transition: "transform .3s ease, box-shadow .3s ease", display: "flex", flexDirection: "column", height: "100%" }}
                    ...
                    <div style={{ padding: "18px 20px 20px", flex: 1, display: "flex", flexDirection: "column" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 16, paddingTop: 14, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ width: 28, height: 28, borderRadius: "50%", background: `linear-gradient(135deg, ${a.catColor}40, rgba(6,182,212,0.3))`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "white" }}>{a.author[0]}</div>
                          <span style={{ fontSize: 12, color: "var(--text-main,#e8eaf0)", opacity: 0.45 }}>{a.author}</span>
                        </div>
                        <span style={{ fontFamily: "monospace", fontSize: 11, color: "var(--text-main,#e8eaf0)", opacity: 0.3 }}>{a.date}</span>
                      </div>
                    </div>
                  </article>
                </Link>
