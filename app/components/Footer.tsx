...
export default function Footer() {
  ...
  const cols = {
    "Belajar":  [
      { label: "Apa itu Bitcoin?",   href: "/artikel/1" },
      { label: "Blockchain 101",     href: "/artikel/2" },
      { label: "Cara Beli BTC",      href: "/artikel/9" },
      { label: "Wallet Aman",        href: "/artikel/4" },
      { label: "Analisis Chart",     href: "/edukasi/6" },
    ],
    "Tools": [
      { label: "Konverter BTC/IDR",  href: "/#konverter" },
      { label: "Harga Realtime",     href: "/#harga" },
      { label: "Semua Artikel",      href: "/artikel" },
      { label: "Semua Modul",        href: "/edukasi" },
    ],
    "Akun": [
  ...
  return (
    <footer style={{ background:"color-mix(in srgb, var(--bg-page,#050810) 95%, transparent)", borderTop:"1px solid rgba(255,255,255,0.05)" }}>
          ...
          <div style={{ display:"flex", gap:8, width:"100%", maxWidth:380 }}>
            <input type="email" placeholder="email@kamu.com"
              style={{ flex:1, padding:"10px 14px", borderRadius:10, fontSize:13, background:"rgba(255,255,255,0.05)",
                border:"1px solid rgba(255,255,255,0.1)", color:"var(--text-main,#e8eaf0)", minWidth:0 }} />
            <button style={{ padding:"10px 16px", borderRadius:10, fontSize:13, fontWeight:700, cursor:"pointer", color:"#000",
              background:"linear-gradient(135deg, var(--color-primary,#f59e0b), #f97316)", whiteSpace:"nowrap", flexShrink:0 }}>
              Subscribe
            </button>
          </div>
        </div>
          ...
          <div className="col-span-2 md:col-span-1">
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16 }}>
              <div style={{ width:34, height:34, borderRadius:10, background:"linear-gradient(135deg, var(--color-primary,#f59e0b), #f97316)",
                display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900, fontSize:13, color:"#000", flexShrink:0 }}>LH</div>
          ...
          {Object.entries(cols).map(([g, items]) => (
            <div key={g}>
              ...
              <ul style={{ listStyle:"none", margin:0, padding:0, display:"flex", flexDirection:"column", gap:9 }}>
                {items.map(item => (
                  <li key={item.label}>
                    <Link href={item.href} style={{ fontSize:13, color:"var(--text-main,#e8eaf0)", opacity:.35, textDecoration:"none", transition:"opacity .15s, color .15s" }}
      ...
      <div style={{ borderTop:"1px solid rgba(255,255,255,0.04)", padding:"16px 0" }}>
        <div className="max-w-7xl mx-auto px-4 font-mono-styled"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-3">
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, opacity:.8, color:"#22c55e" }}>
