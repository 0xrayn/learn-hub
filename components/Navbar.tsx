  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 h-0.5 z-[60]"
        style={{ background: "linear-gradient(90deg, transparent, #e8002d, #ff4d6d, #e8002d, transparent)" }} />

      <nav className={`fixed top-0.5 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-base-100/80 backdrop-blur-xl border-b border-white/5 shadow-2xl"
          : "bg-transparent"
      }`}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8 flex items-center justify-between h-16">
          ...
          <a href="#beranda" className="flex items-center gap-2.5 group">
            <div className="relative">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center red-glow-sm"
          ...
          <ul className="hidden md:flex items-center gap-1">
            {NAV_LINKS.map((l) => (
              <li key={l.href}>
                <a href={l.href}
                  className="px-4 py-2 rounded-xl text-sm font-medium text-base-content/60 hover:text-base-content hover:bg-white/5 transition-all duration-200 relative group">
                  {l.label}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="dropdown dropdown-end">
              <div tabIndex={0} role="button"
                className="btn btn-ghost btn-sm gap-1.5 border border-white/10 hover:border-red-500/30 hover:bg-red-500/10 transition-all">
                <Palette size={14} style={{ color: "#e8002d" }} />
                <span className="hidden sm:inline text-xs font-medium">Tema</span>
              </div>
              <div tabIndex={0}
                className="dropdown-content z-[100] p-3 mt-3 w-56 rounded-2xl border border-white/10"
                ...
                {THEMES.map((t) => (
                  <button key={t.id} onClick={() => changeTheme(t.id)}
                    ...
                    style={theme === t.id ? { background: "rgba(232,0,45,0.2)", border: "1px solid rgba(232,0,45,0.3)" } : {}}>
                    <span className="text-base">{t.emoji}</span>
                    <span>{t.label}</span>
                    {theme === t.id && <Zap size={12} className="ml-auto" style={{ color: "#e8002d" }} />}
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* CTA */}
            <a href="#tentang"
              className="hidden sm:flex btn btn-sm font-semibold text-white border-0 red-glow-sm hover:scale-105 transition-transform"
              style={{ background: "linear-gradient(135deg, #e8002d, #ff4d6d)" }}>
              Mulai Belajar
            </a>
        ...
        {/* Mobile dropdown */}
        {menuOpen && (
          <div className="md:hidden border-t border-white/5 px-5 py-4 flex flex-col gap-1"
            style={{ background: "rgba(5,5,8,0.97)", backdropFilter: "blur(20px)" }}>
            {NAV_LINKS.map((l) => (
              <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)}
                className="px-4 py-2.5 rounded-xl text-sm font-medium text-base-content/70 hover:text-white hover:bg-white/5 transition-colors">
                {l.label}
              </a>
            ))}
            <a href="#tentang" onClick={() => setMenuOpen(false)}
              className="mt-2 btn btn-sm text-white border-0"
              style={{ background: "linear-gradient(135deg, #e8002d, #ff4d6d)" }}>
              Mulai Belajar
            </a>
          </div>
